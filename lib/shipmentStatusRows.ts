/**
 * Подготовка позиций поставки к отображению по статусам.
 *
 ...
 */
export function buildShipmentStatusRows(positions: readonly Position[]): ViewRow[] {
  const grouped = groupByStatusLabel([...positions]);
  const rows: ViewRow[] = [];
