...
describe("toShipmentPositions", () => {
  it("преобразует raw item в позицию с размерами, суммой, себестоимостью и стабильным id", () => {
    ...
    expect(positions[0].sizes).toMatchObject({
      XS: 2,
      S: 3,
      M: 0,
      L: 0,
  ...
  it("не превращает sample в одну штуку, если размеры уже задают количество", () => {
    ...
    expect(positions[0].sum).toBe(180);
  });
  ...
  it("не включает paidPreviously/noPayment позиции в сумму", () => {
    ...
    expect(positions.map((position) => position.isPayable)).toEqual([false, false]);
    expect(positions.map((position) => position.sum)).toEqual([null, null]);
  });
  ...
  it.each([undefined, "", "   "])(
    ...
    (status) => {
      ...
      expect(positions[0].statusLabel).toBe("Получено, оплачено ✅");
    }
  );
  ...
  it("сохраняет непустой статус позиции вместо статуса поставки", () => {
    ...
    expect(positions[0].statusLabel).toBe("В производстве 🛠️");
  });
