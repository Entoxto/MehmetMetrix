...
- Парсер берёт `cost` из колонки N только при положительном `Курсе списания` в J. При пустом, нулевом или отрицательном курсе формула N может временно содержать только карго до оплаты товара
...
4. Извлекает `cost` из колонки N ("Себестоимость с учётом карго") — себестоимость в рублях, но только если `Курс списания` в колонке J положительный
