...
│   └── shipment.ts         # Типы для поставок: ShipmentRawItem, ShipmentConfig, ShipmentWithItems, ShipmentStatus
│
├── lib/                    # Бизнес-логика
...
> **Примечание:** Статусы в коде маппятся из текстовых значений Excel через функции `normalize_position_status()` и `normalize_shipment_status()` в `Excel/utils.py`. Благодаря выпадающим спискам (Data Validation) в Excel, маппинг является жестким и безопасным — риск опечатки исключен на уровне источника данных. Фильтрация в разделе "Деньги" по слову "оплач" также опирается на строго регламентированные значения из Excel.
...
3. **Жёсткий маппинг:** Функции `normalize_position_status()` и `normalize_shipment_status()` в `Excel/utils.py` ожидают конкретные регламентированные строки из выпадающих списков Excel. Любое изменение формулировок в Excel должно сопровождаться обновлением констант маппинга в этих функциях.
