...
6. **Обрезка изображений** — в каталоге и меню используется `objectFit: "cover"` с `objectPosition: "top center"` для заполнения контейнера с сохранением пропорций (обрезка только снизу). В детальном просмотре на мобильных используется `cover` для растягивания на всю ширину, на десктопе — `contain` для показа изображения полностью без обрезки
...
- **Детальный просмотр (ProductDetail):**
  - **На мобильных устройствах:** используется `objectFit: "cover"` с `objectPosition: "top center"` — изображение растягивается на всю ширину без боковых рамок, обрезка только снизу.
...
- **Детальный просмотр (ProductDetail):** 
  - Мобильные: 
    - Фиксированная высота 300px, ширина 100%
