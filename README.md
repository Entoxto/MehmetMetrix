...
> **Примечание:** В JSON файле (`shipments.json`) статус указывается как строка (например, `"inProgress"`, `"receivedPaid"`). В TypeScript коде используется enum `ShipmentStatus` из `types/shipment.ts`. Значения enum соответствуют строкам в JSON, поэтому сравнение `shipment.status === ShipmentStatus.receivedPaid` работает корректно.
...
6. **Обрезка изображений** — используется `objectFit: "cover"` с `objectPosition: "top center"` для заполнения контейнера с сохранением пропорций. Изображения выравниваются по верхнему краю, обрезка происходит только снизу, верхняя часть всегда видна полностью
...
**Принцип работы:**
- Используется `objectFit: "cover"` — изображение заполняет контейнер, сохраняя пропорции (обрезка неизбежна при несоответствии пропорций)
